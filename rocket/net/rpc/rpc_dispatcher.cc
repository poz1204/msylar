...
//  requset 转调用 proto 中调用的方法， proto_res 转换为 tinypb_protocol
void RpcDispatcher::dispatch(AbstractProtocol::s_ptr requset, AbstractProtocol::s_ptr response, TcpConnection* connection) {
    std::shared_ptr<TinyPBProtocol> req_protocol = std::dynamic_pointer_cast<TinyPBProtocol>(requset);
    ...
    INFOLOG("msg_id[%s], get rpc requset[%s]", req_protocol->m_msg_id.c_str(), req_msg->ShortDebugString().c_str());

    google::protobuf::Message* rsp_msg = service->GetResponsePrototype(method).New();

    RpcController rpcController;
    IPNetAddr::s_ptr local_addr = std::make_shared<IPNetAddr>("127.0.0.1", 1234); // todo 封装
    rpcController.SetLocalAddr(connection->getLocalAddr());
    rpcController.SetPeerAddr(connection->getPeerAddr());
    rpcController.SetMsgId(req_protocol->m_msg_id);

    RunTime::GetRunTime()->m_msgid = req_protocol->m_msg_id;  // 用于app_log
...
bool RpcDispatcher::parseServiceFullName(const std::string& full_name, std::string& service_name, std::string& method_name) {
    ...
    INFOLOG("parse, service_name: [%s], method_name: [%s] from full+name: [%s]", service_name.c_str(), method_name.c_str(), full_name.c_str());
    return true;
}
